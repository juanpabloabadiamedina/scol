...
const (
	clientIdentifier = "scol" // Client identifier to advertise over the network
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the scol command line interface")
...
func init() {
	...
	app.Copyright = "Copyright 2013-20220 The go-ethereum Authors and SCOL Authors"
...
func prepare(ctx *cli.Context) {
	...
	case ctx.GlobalIsSet(utils.TestnetFlag.Name):
		log.Info("Starting scol on testnet...")

	case ctx.GlobalIsSet(utils.DeveloperFlag.Name):
		log.Info("Starting scol in ephemeral dev mode...")

	case !ctx.GlobalIsSet(utils.NetworkIdFlag.Name):
		log.Info("Starting scol on mainnet...")
	}
...
func startNode(ctx *cli.Context, stack *node.Node, backend ethapi.Backend) {
	...
	if ctx.GlobalBool(utils.MiningEnabledFlag.Name) || ctx.GlobalBool(utils.DeveloperFlag.Name) {
		...
		if !ok {
			utils.Fatalf("Scol service not running: %v", err)
		}
